...
<tbody class="" >
    ...
    @foreach ($letters as $letter)
      <tr wire:loading.class.delay="" class="" wire:key="row-0-4Of9aF3orQYiqBL2xp3j">
        ...
        <td>
               <form action="{{ route('forms.order.destroy', $letter) }}" method="POST">
                @csrf
                @method('DELETE')
...
//     const lettersTable = $('#lettersTable');
    ...
    const handleDraftSearch = function() {
        ...
        $.ajax({
            url: `/super-admin/orders/search?${params}`,
            type: 'GET',
            dataType: 'html',
      ...
      function downloadPdf(url) {
        ...
        function signedFile(letterId, fileType) {
                              ...
                              if (fileType === 'pdf') {
                                  ...
                                  url = "{{ route('orders.download_signed', ':id') }}".replace(':id', letterId);
    ...
    var previewUrl = '{{ route('order.previews', ':id') }}';
    ...
    fetch(previewUrl)
        ...
        .then(htmlContent => {
            ...
            modal._element.addEventListener('hidden.bs.modal', function () {
                window.location.href = '{{ route('super.admin.total.orders', ':id') }}'.replace(':id', letterId);
            });
        })
        .catch(error => {
